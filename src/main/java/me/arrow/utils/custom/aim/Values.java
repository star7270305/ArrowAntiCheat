package me.arrow.utils.custom.aim;
...
//unused, i don't even know where i got this from
...
public class Values {
    ...
    public static double percentToMcpValue(int percent) {
        percent = Math.max(0, Math.min(200, percent));

        Double direct = SENSITIVITY_MCP_VALUES.get(percent);
        if (direct != null && (percent == 0 || !isPlaceholder(direct))) {
            return direct;
        }

        // find nearest lower present
        int lower = percent;
        while (lower >= 0) {
            Double v = SENSITIVITY_MCP_VALUES.get(lower);
            if (v != null && (lower == 0 || !isPlaceholder(v))) break;
            lower--;
        }

        // find nearest upper present
        int upper = percent;
        while (upper <= 200) {
            Double v = SENSITIVITY_MCP_VALUES.get(upper);
            if (v != null && (upper == 0 || !isPlaceholder(v))) break;
            upper++;
        }

        if (lower < 0 && upper > 200) return 0.0; // nothing at all (shouldn't happen)
        if (lower < 0) return SENSITIVITY_MCP_VALUES.get(upper);
        if (upper > 200) return SENSITIVITY_MCP_VALUES.get(lower);
        if (lower == upper) return SENSITIVITY_MCP_VALUES.get(lower);

        double vL = SENSITIVITY_MCP_VALUES.get(lower);
        double vU = SENSITIVITY_MCP_VALUES.get(upper);

        // linear interpolation between lower and upper percent keys
        double t = (percent - lower) / (double) (upper - lower);
        return vL + (vU - vL) * t;
    }

    /**
     * Convert an MCP mapping value back to the percent (0..200) on our scale.
     * We sort all present mapping entries by value and then locate the two entries that bracket
     * the given mcpValue; linear interpolation between their percent keys gives a smooth inverse.
     */
    public static int mcpValueToPercent(double mcpValue) {
        // collect present (non-placeholder) entries
        List<Entry<Integer, Double>> present = new ArrayList<>();
        for (Entry<Integer, Double> e : SENSITIVITY_MCP_VALUES.entrySet()) {
            if (!isPlaceholder(e.getValue()) || e.getKey() == 0) {
                present.add(e);
            }
        }
        ...
        Collections.sort(present, Comparator.comparingDouble(Entry::getValue));

        // bounds
        Entry<Integer, Double> first = present.get(0);
        Entry<Integer, Double> last = present.get(present.size() - 1);

        if (mcpValue <= first.getValue()) return first.getKey();
        if (mcpValue >= last.getValue()) return last.getKey();

        // find bracket
        for (int i = 0; i < present.size() - 1; i++) {
            Entry<Integer, Double> a = present.get(i);
            Entry<Integer, Double> b = present.get(i + 1);
            double va = a.getValue();
            double vb = b.getValue();
            if (mcpValue >= va && mcpValue <= vb) {
                if (Math.abs(vb - va) < 1e-12) return a.getKey();
                double t = (mcpValue - va) / (vb - va);
                double percent = a.getKey() + t * (b.getKey() - a.getKey());
                return (int) Math.round(percent);
            }
        }

        // fallback - nearest by absolute diff
        int bestKey = first.getKey();
        double bestDiff = Math.abs(mcpValue - first.getValue());
        for (Entry<Integer, Double> e : present) {
            double d = Math.abs(mcpValue - e.getValue());
            if (d < bestDiff) {
                bestDiff = d;
                bestKey = e.getKey();
            }
    ...
    /**
     * Helper: treat certain 0.0 entries as placeholders (missing). We consider an entry as placeholder
     * if it == 0.0 and the key != 0 (since 0% legitimately maps to 0.0).
     */
    private static boolean isPlaceholder(Double v) {
        return v == null || (v.doubleValue() == 0.0);
    }

    // Optional convenience: convert Karhu mouseDelta (float) -> percent (0..200)
    public static int mouseDeltaToPercent(double mouseDelta) {
        return (int) Math.floor(mouseDelta * 200.0);
    }
}
