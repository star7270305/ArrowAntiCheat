...
public class RayTrace {
    ...
    public void highlight(World world, double blocksAway, double accuracy) {
        for (Vector position : traverse(blocksAway, accuracy)) {
            world.playEffect(position.toLocation(world), (PacketEvents.getAPI().getPlayerManager().getClientVersion(world).isNewerThanOrEquals(ClientVersion.V_1_13) ? Effect.SMOKE : Effect.valueOf("COLOURED_DUST")), 0);
        }
    }
}
